...
import storeBookingAgentNote from "requests/storeBookingAgentNote";
...
import markCargoNotesSeen from "requests/markCargoNotesSeen";
import { useEffect, useState } from "react";
import { toast } from "react-toastify";
import { MentionsInput, Mention } from 'react-mentions'
import { Link } from "react-router-dom/cjs/react-router-dom.min";
import markBookNotesSeen from "requests/markBookNotesSeen";
...
const BookNotesModal = ({ canAddNote, openNotesModalModal, showNotesModal, closeNotesModalModal, activeNotes, setActiveNotes, books, setBooks, auth }) => {
    // const { auth } = useContext(AuthContext);
    const [storeBookingAgentReq] = useRequest(storeBookingAgentNote);
    const [deleteBookingAgentNoteReq] = useRequest(deleteBookingAgentNote);
    const [markCargoNotesSeenReq] = useRequest(markCargoNotesSeen);
    const [markBookNotesSeenReq] = useRequest(markBookNotesSeen);
    ...
    useEffect(() => {
        if (activeNotes.tracking !== undefined) {
            if (activeNotes.notes[0]?.seen === 0) {
                markCargoNotesSeenReq({ tracking: activeNotes.notes[0].tracking })
                    .then((response) => {
                        let crgs = [...books];
                        setBooks(crgs.map(x => {
                            if (x.tracking_number === activeNotes.tracking) {
                                x.web_notes[0].seen = 1;
                            }
                            return x;
                        }));
                    });
            ...
            if (activeNotes.notes[0]?.seen === 0) {
                markBookNotesSeenReq({ book_id: activeNotes.notes[0].book_id })
                    .then((response) => {
                        let crgs = [...books];
                        setBooks(crgs.map(x => {
                            if (x.id === activeNotes.id) {
                                x.book_notes[0].seen = 1;
                            }
                            return x;
                        }));
                    });
    ...
    const handleStoreNote = () => {
        setStoringNote(true);
        storeBookingAgentReq({ tracking: activeNotes?.tracking, booking: activeNotes?.booking, body: newNote.body })
            .then((response) => {
                        ...
                        <div className="row  d-flex justify-content-center">
                            <div className="col-md-12">
                                ...
                                {activeNotes && activeNotes.notes && activeNotes.notes.length > 0 && activeNotes.notes.sort((a, b) => new Date(b.created_at) - new Date(a.created_at)).map((note, i) => (
                                    <div className="card p-2 mb-2" key={i} style={{ backgroundColor: (note.body.includes(auth.staff.username) ? '#ffefd1' : 'white') }}>
                                            ...
                                            <div className="user d-flex flex-row align-items-center">
                                                <span><small className="font-weight-bold text-primary">{note.customer_id !== null && (<i className="bi bi-person-badge me-1"></i>)}
                                                    {note.customer_id !== null ? (
                                                        <Link role="button" to={`/manage/client/${note.customer_id}`}>{note.username}</Link>
                                                    ) : (
                                                        <Link role="button" to={`/manage/agent/${note.staff_id}`}>{note.username}</Link>
                                                    )}
                                                </small>
